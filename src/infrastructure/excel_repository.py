...
class ExcelRepository(AuthRepository, EmailRepository):
    """
    Concrete repository implementation using Excel files.
    ...
    def __init__(self, auth_file: str, email_file: str, encryption_service: EncryptionService):
        ...
        # Ensure Excel files exist with correct headers
        self._check_and_create_files()
        logger.info("Excel repository initialized.")

    def _check_and_create_files(self):
        """Creates Excel files with headers if they don't exist."""
        try:
            if not os.path.exists(self.auth_file):
                df = pd.DataFrame(columns=['code', 'is_used'])
                df.to_excel(self.auth_file, index=False, engine='openpyxl')
                logger.info(f"Created new Excel file: {self.auth_file}")
            ...
            if not os.path.exists(self.email_file):
                ...
                df.to_excel(self.email_file, index=False, engine='openpyxl')
                logger.info(f"Created new Excel file: {self.email_file}")
    ...
    def get_auth_code(self, code: str) -> Optional[AuthCode]:
        """Reads auth codes from Excel and finds a matching unused code."""
        try:
            df = pd.read_excel(self.auth_file,engine='openpyxl')
            ...
            logger.error(f"Error getting auth code from Excel: {e}")
            return None

    def update_auth_code_status(self, code: str, is_used: bool) -> None:
        """Updates the status of an auth code in the Excel file."""
        try:
            df = pd.read_excel(self.auth_file)
            df.loc[df['code'] == code, 'is_used'] = is_used
            df.to_excel(self.auth_file, index=False)
    ...
    def add_email(self, email: EmailMessage) -> None:
        """Appends a new email record to the emails Excel file."""
        try:
            df_emails = pd.read_excel(self.email_file, engine='openpyxl')
            ...
            df_emails.to_excel(self.email_file, index=False)
            logger.info(f"Email with ID {email.id} queued for sending.")
        except Exception as e:
            logger.error(f"Error adding email to Excel: {e}")
            raise

    def get_emails_to_send(self, limit: int) -> List[EmailMessage]:
        """Retrieves unsent emails from the Excel file."""
        emails = []
        try:
            df = pd.read_excel(self.email_file)
...
def recreate_auth_file(auth_file):
    """Recreates the auth_codes.xlsx file with valid Excel format."""
    data = {
        'code': ['ABC123', 'DEF456', 'GHI789', 'JKL012', 'MNO345', 'PQR678', 'STU901', 'VWX234', 'YZA567', 'BCD890'],
        'is_used': [False] * 10
    }
    df = pd.DataFrame(data)
    df.to_excel(auth_file, index=False, engine='openpyxl')

# Call this function to recreate the file
recreate_auth_file('auth_codes.xlsx')
